 ...
 * Genera un ID único
 */
function generateId(prefix = 'patient') {
 ...
 */
function extractFechaNacimiento(text) {
 ...
 */
function extractDireccion(text) {
 ...
 */
function extractDiagnosticos(text) {
    const diagnosticos = {
        saludMental: '',
        morbilidadMedica: '',
    ...
    if (diagSection) {
        ...
        for (const keyword of medicalKeywords) {
            ...
            if (match) {
                if (diagnosticos.morbilidadMedica)
                    diagnosticos.morbilidadMedica += '; ';
 ...
 */
function extractSexo(text, nombre) {
    ...
    for (const m of nombresMasculinos) {
        if (nombreLower.includes(m))
            return 'Masculino';
    }
    for (const f of nombresFemeninos) {
        if (nombreLower.includes(f))
            return 'Femenino';
    }
 ...
 */
function extractOcupacion(text) {
    const occupations = [
 ...
 */
function extractClinicalNotes(text, patientId) {
    const notes = [];
 ...
 */
function parsePatientFile(filePath, fichaNumber) {
    try {
